...
public:
	...
	UnderlayingType MagnitudeSquared() const
	{
		return (X * X) + (Y * Y);
	}
...
public:
    ...
    std::string ToString() const {
        return std::format("({:f}, {:f}, {:f})", Pitch, Yaw, Roll);
    }
};
...
public:
	...
	constexpr FLinearColor(float r, float g, float b, float a = 1.0f) : R(r), G(g), B(b), A(a) {}
	constexpr FLinearColor(const FColor& Color) : R(Color.R / 255.0f), G(Color.G / 255.0f), B(Color.B / 255.0f), A(Color.A / 255.0f) {}

    // 从十六进制颜色值构造 (例如: 0xd1814e, 0xRRGGBB)
    constexpr FLinearColor(uint32_t hexColor, float alpha = 1.0f)
    ...
    std::string ToString() const {
        return std::format("({:f}, {:f}, {:f}, {:f})", R, G, B, A);
    }
};
